...
def train():
  ...
  for episode in range(config.NUM_EPISODES):
    ...
    # Offline RL: Train only on pre-loaded data from buffer
    # No new data generation - use fixed dataset
    episode_sampled_rewards = []
    ...
    for step in range(config.MAX_STEPS_PER_EPISODE):
      ...
      # sample in the PER buffer (from pre-loaded data only)
      ...
      # Track sampled rewards for logging
      episode_sampled_rewards.append(r_b.mean().item())
    ...
    if episode % config.LOG_INTERVAL == 0:
      avg_sampled_reward = np.mean(episode_sampled_rewards) if episode_sampled_rewards else 0.0
      print(f"Epi : {episode}, Total Steps : {total_steps}, Avg Sampled Reward : {avg_sampled_reward:.4f}")
